...
export default class EquipmentManager extends cc.Component {
    ...
    getRandomDesc(data: EquipmentData): EquipmentDescData {
        ...
        let level = 0;
        let criticalStrikeRate = this.getRandomQuality(0, 100 - data.criticalStrikeRate);
        level = criticalStrikeRate.y > level ? criticalStrikeRate.y : level;
        desc.prefix += criticalStrikeRate.y > 0 ? 'criticalstrike ' : '';
        desc.color = this.getMixColor('#000000'
            , criticalStrikeRate.y > 0 ? EquipmentManager.COLOR_CRITICALSTRIKE : '#000000');

        let damageMin = this.getRandomQuality(0, 10);
        level = damageMin.y > level ? damageMin.y : level;

        let damageMax = this.getRandomQuality(damageMin.x, damageMin.x + 10);
        level = damageMax.y > level ? damageMax.y : level;
        desc.prefix += damageMax.y > 0 ? 'strong ' : '';
        desc.color = this.getMixColor(desc.color
            , damageMax.y > 0 ? EquipmentManager.COLOR_POWERFUL : '#000000');

        let defence = this.getRandomQuality(0, 10);
        level = defence.y > level ? defence.y : level;
        desc.prefix += defence.y > 0 ? 'stable ' : '';
        desc.color = this.getMixColor(desc.color
            , defence.y > 0 ? EquipmentManager.COLOR_STABLE : '#000000');

        let lifeDrain = this.getRandomQuality(0, 100);
        level = lifeDrain.y > level ? lifeDrain.y : level;
        desc.prefix += lifeDrain.y > 0 ? 'drain ' : '';
        desc.color = this.getMixColor(desc.color
            , lifeDrain.y > 0 ? EquipmentManager.COLOR_LIFEDRAIN : '#000000');

        //为了防止过于imba只有1/10的可能回血
        let lifeRecovery = Math.random() < 0.1 ? this.getRandomQuality(0, 5) : cc.v2(0, 0);
        level = lifeRecovery.y > level ? lifeRecovery.y : level;
        desc.prefix += lifeRecovery.y > 0 ? 'recovery ' : '';
        desc.color = this.getMixColor(desc.color
            , lifeRecovery.y > 0 ? EquipmentManager.COLOR_RECOVERY : '#000000');

        let moveSpeed = this.getRandomQuality(0, 300 - data.moveSpeed);
        level = moveSpeed.y > level ? moveSpeed.y : level;
        desc.prefix += moveSpeed.y > 0 ? 'fast ' : '';
        desc.color = this.getMixColor(desc.color
            , moveSpeed.y > 0 ? EquipmentManager.COLOR_MOVESPEED : '#000000');

        let attackSpeed = this.getRandomQuality(0, 300 - data.attackSpeed);
        level = attackSpeed.y > level ? attackSpeed.y : level;
        desc.prefix += attackSpeed.y > 0 ? 'quick ' : '';
        desc.color = this.getMixColor(desc.color
            , attackSpeed.y > 0 ? EquipmentManager.COLOR_ATTACKSPPED : '#000000');

        let dodge = this.getRandomQuality(0, 60 - data.dodge);
        level = dodge.y > level ? dodge.y : level;
        desc.prefix += dodge.y > 0 ? 'agile ' : '';
        desc.color = this.getMixColor(desc.color
            , dodge.y > 0 ? EquipmentManager.COLOR_DODGE : '#000000');

        let health = this.getRandomQuality(0, 50);
        level = health.y > level ? health.y : level;
        desc.prefix += health.y > 0 ? 'healthy ' : '';
        desc.color = this.getMixColor(desc.color
            , health.y > 0 ? EquipmentManager.COLOR_HEALTHY : '#000000');
    ...
    //x:qulity y:level 
    getRandomQuality(min: number, max: number): cc.Vec2 {
        let per = (max - min) / 5;
