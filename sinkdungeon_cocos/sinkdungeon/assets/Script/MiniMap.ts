...
export default class MiniMap extends cc.Component {
    ...
    onLoad () {
        ...
        for (let i = 0; i < this.size; i++) {
            ...
            for (let j = 0; j < this.size; j++) {
                ...
                this.map[i][j].position = cc.v2(i * 16, j * 16);
    ...
    changeMap(x:number,y:number):void{
		...
		for (let j = 0; j < this.size; j++) {
			for (let i = 0; i < this.size; i++) {
				let isFound = Logic.rectDungeon.map[i][j].isFound;
				let state = Logic.rectDungeon.map[i][j].state;
				let roomType = Logic.rectDungeon.map[i][j].roomType;
				this.map[i][j].color = this.getColor (MiniMap.ColorLevel.HIDE);
				if (isFound) {
                    this.map [i][j].color = new cc.Color(220,220,220);
	...
	getColor(t:number):cc.Color{
		...
		case MiniMap.ColorLevel.NORMAL_BOSS:
			color = new cc.Color(255,0,0,200);
			break;
