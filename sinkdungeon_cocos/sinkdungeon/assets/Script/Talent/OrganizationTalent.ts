    ...
    protected doSkill() {
        if (this.player.data.AvatarData.organizationIndex == AvatarData.GURAD) {
            ...
            if (this.player.dungeon.nonPlayerManager.isPetAlive()) {
                ...
                Utils.toast(`宠物的力量增强了${Logic.playerData.OilGoldData.level * 5 + this.data.useCount*3}点血量上限，血量上限为${this.player.dungeon.nonPlayerManager.pet.data.Common.maxHealth}，攻击力为${d.getAttackPoint().getTotalDamage()}`,false,true)
            } else {
                ...
                Utils.toast(`你召唤了宠物${data.nameCn}：血量上限为${this.player.dungeon.nonPlayerManager.pet.data.Common.maxHealth}，攻击力为${data.getAttackPoint().getTotalDamage()}`,false,true)
