...
export default class ParallexBackground extends cc.Component {
    ...
    player:Player;
    init(){
        this.layer1.getComponent(cc.Sprite).spriteFrame = Logic.spriteFrames[`chapter0${Logic.chapterName}layer1`];
        this.layer2.getComponent(cc.Sprite).spriteFrame = Logic.spriteFrames[`chapter0${Logic.chapterName}layer2`];
    }
    getPlayer():Player{
        if(!this.player){
