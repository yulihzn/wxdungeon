...
export class DISGUISE extends BaseNonPlayerActorState {
    ...
    exit(entity: NonPlayer): void {
        super.exit(entity)
        Utils.log(`${entity.actorName()}${entity.node.uuid}(DISGUISE):exit`)
    }
    event(entity: NonPlayer, event: FsmEvent): boolean {
        super.event(entity, event)
        Utils.log(`${entity.actorName()}${entity.node.uuid}(DISGUISE):event`)
        return true
    }
}
/** 冲刺：待机*/
...
export default class NonPlayerActorState {
    ...
    static WALK = new WALK('WALK')
    static ATTACK = new ATTACK('ATTACK')
    static IDLE = new IDLE('IDLE')
    static DISGUISE = new DISGUISE('DISGUISE')
    static DIED = new DIED('DIED')
    static DODGE = new DODGE('DODGE')
    static HURT = new HURT('HURT')
    static PRPARE = new PRPARE('PRPARE')
    static SHOW = new SHOW('SHOW')
    static BLINK = new BLINK('BLINK')
    static FALL = new FALL('FALL')
    static TALK = new TALK('TALK')
