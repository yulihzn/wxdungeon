    ...
    onColliderStay(other: CCollider, self: CCollider) {
        ...
        if (target) {
            if (this.isFall && this.isValid) {
                this.isFall = false
                target.takeDamage(new DamageData(2), FromData.getClone('落石', 'stone', this.node.position))
            }
        }
        ...
        if (monster) {
            if (this.isFall && this.isValid) {
                this.isFall = false
                monster.takeDamage(new DamageData(2), FromData.getClone('落石', 'stone', this.node.position))
            }
        }
    }
    // update (dt) {}
