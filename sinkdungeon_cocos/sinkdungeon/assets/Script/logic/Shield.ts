...
export default class Shield extends cc.Component {
    ...
    private static readonly DEFAULT_POS = cc.v3(-8, 48)
    private static readonly TRANSFORM_POS = cc.v3(32, 44)
    private static readonly DEFEND_POS = cc.v3(24, 40)
    ...
    private playIdle() {
        ...
        cc.log(`等待  zIndex:${this.node.zIndex}`)
    ...
    private playParry(isFaceRight: boolean) {
        ...
        let dp = Shield.DEFEND_POS.clone()
        let tp = Shield.TRANSFORM_POS.clone()
        if (!isFaceRight) {
            dp.x = -dp.x
            tp.x = -tp.x
        }
        cc.log(`举起 zIndex:${this.node.zIndex}`)
        cc.tween(this.node)
            .to(duration, { position: tp })
            .call(() => {
                this.node.zIndex = this.avatarZindex + Shield.ZOFFSET
    ...
    private playDefend() {
        ...
        cc.log(`防御 zIndex:${this.node.zIndex}`)
    }
    private playPutDown(isFaceRight: boolean) {
        ...
        let dp = Shield.DEFAULT_POS.clone()
        let tp = Shield.TRANSFORM_POS.clone()
        if (!isFaceRight) {
            dp.x = -dp.x
            tp.x = -tp.x
        }
        cc.log(`放下 zIndex:${this.node.zIndex}`)
        cc.tween(this.node)
            .to(duration, { position: tp })
            .call(() => {
                this.node.zIndex = this.avatarZindex - Shield.ZOFFSET
                this.sprite.node.color = cc.color(32, 32, 32)
            })
            .to(duration, { position: dp })
            .call(() => {
                this.playIdle()
    ...
    public changeZIndex(avatarZindex: number, isFaceRight: boolean) {
        if (this.isAniming) {
            return
        }
        this.avatarZindex = avatarZindex
        let isDefending = this.status == Shield.STATUS_DEFEND
        let currentIndex = avatarZindex + (isDefending ? Shield.ZOFFSET : -Shield.ZOFFSET)
        this.sprite.node.color = isDefending ? cc.Color.WHITE : cc.color(32, 32, 32)
        this.node.zIndex = currentIndex
        let p = isDefending ? Shield.DEFEND_POS.clone() : Shield.DEFAULT_POS.clone()
        if (!isFaceRight) {
            p.x = -p.x
        }
