...
export default class BuildingManager extends BaseManager {
    ...
    public async addBuildingsFromMap(
        dungeon: Dungeon,
        mapData: string[][],
        mapDataStr: string,
        indexPos: cc.Vec3,
        ...
        if (!mapDataStr || mapDataStr.length < 1 || mapDataStr == 'undefined') {
            ...
            let dn = this.addBuilding(await Logic.getBuildings(BuildingManager.DARKNESS), indexPos)
            ...
            //生成踏板
            let foot = this.addBuilding(await Logic.getBuildings(BuildingManager.FOOTBOARD), indexPos)
            foot.zIndex = IndexZ.FLOOR
            this.footboards.push(foot.getComponent(FootBoard))
        } else if (mapDataStr == '@S') {
            //生成存档点
            let save = this.addBuilding(await Logic.getBuildings(BuildingManager.SAVEPOINT), indexPos)
            ...
            //生成营火
            let camp = this.addBuilding(await Logic.getBuildings(BuildingManager.CAMPFIRE), indexPos)
            ...
            this.addBuilding(await Logic.getBuildings(BuildingManager.AIRTRANSPORTMODEL), indexPos)
        } else if (this.isFirstEqual(mapDataStr, 'G')) {
            //生成炮台
            let em = this.addBuilding(await Logic.getBuildings(BuildingManager.EMPLACEMENT), indexPos).getComponent(Emplacement)
            ...
            //生成通风管
            let p = this.addBuilding(await Logic.getBuildings(BuildingManager.WENTLINE), indexPos).getComponent(MgWentLine)
            ...
            //生成墙缝
            let p = this.addBuilding(await Logic.getBuildings(BuildingManager.CRACK), indexPos).getComponent(MgCrack)
            ...
            //生成顶部栏
            let head = this.addBuilding(await Logic.getBuildings(BuildingManager.DECORATIONOVERHEAD), indexPos)
            if (mapDataStr == 'O1') {
                head.angle = 90
            }
            head.opacity = 80
            ...
            if (Logic.isCheatMode) {
                ...
                let p = this.addBuilding(await Logic.getBuildings(BuildingManager.PORTAL), indexPos)
            ...
            let node = this.addBuilding(await Logic.getBuildings(BuildingManager.SHIPSTAIRS), indexPos)
            node.setScale(16)
            ...
            //生成店主
            this.addBuilding(await Logic.getBuildings(BuildingManager.SHOP), indexPos)
        } else if (mapDataStr == 'S2') {
            //生成有家
            let mart = this.addBuilding(await Logic.getBuildings(BuildingManager.SHOPMART), indexPos)
            mart.zIndex += 10
            ...
            //生成收银台
            this.addBuilding(await Logic.getBuildings(BuildingManager.MARTCASHIER), indexPos)
            ...
            //生成餐桌
            this.addBuilding(await Logic.getBuildings(BuildingManager.MARTTABLE), indexPos)
        } else if (mapDataStr == 'T000') {
            //生成陷阱
            this.addBuilding(await Logic.getBuildings(BuildingManager.TRAP), indexPos)
        } else if (mapDataStr == 'T002') {
            //生成电锯,占据5个格子
            let saw = this.addBuilding(await Logic.getBuildings(BuildingManager.SAW), indexPos)
            ...
            let wallpaint = this.addBuilding(await Logic.getBuildings(BuildingManager.WALLPAINT), indexPos).getComponent(WallPaint)
            ...
            let decorationWall = this.addBuilding(await Logic.getBuildings(BuildingManager.DECORATIONWALL), indexPos).getComponent(DecorationWall)
            ...
            if (mapDataStr == 'Z0' || mapDataStr == 'Z1') {
                let p = this.addBuilding(await Logic.getBuildings(BuildingManager.ROOMBED), indexPos, CCollider.AUDIO_MATERIAL.WOOD)
    ...
    private async addLadder(mapDataStr: string, indexPos: cc.Vec3) {
        let p = this.addBuilding(await Logic.getBuildings(BuildingManager.LADDER), indexPos)
        let ladder = p.getComponent(Ladder)
        p.zIndex = IndexZ.getActorZIndex(p.position.add(cc.v3(0, 120)))
        ladder.init(mapDataStr)
    }
    public async addBuildingsFromSideMap(mapDataStr: string, mapData: string[][], indexPos: cc.Vec3, levelData: LevelData) {
    ...
    private addStairs(mapDataStr: string, indexPos: cc.Vec3) {
        ...
        map.forEach(async (value: string, key: string) => {
            if (this.hasThe(mapDataStr, key)) {
                let building = this.addBuilding(await Logic.getBuildings(value), indexPos).getComponent(Stairs)
                building.data.z = 0
    ...
    private async addNormalBuilding(dungeon: Dungeon, mapDataStr: string, prefix: string, indexPos: cc.Vec3) {
        let building = this.addBuilding(
            await Logic.getBuildings(BuildingManager.NORMAL_BUILDING),
            indexPos,
            CCollider.AUDIO_MATERIAL.METAL,
            `${prefix}${mapDataStr.substring(1)}`
        ).getComponent(NormalBuilding)
    ...
    private async addInteractBuilding(mapDataStr: string, indexPos: cc.Vec3) {
        let prefab = await Logic.getBuildings(BuildingManager.INTERACTBUILDING)
    ...
    private async addBox(mapDataStr: string, indexPos: cc.Vec3) {
        let box = this.addBuilding(await Logic.getBuildings(BuildingManager.BOX), indexPos, CCollider.AUDIO_MATERIAL.WOOD, '', true)
    ...
    private async addShopTable(indexPos: cc.Vec3) {
        let table = this.addBuilding(await Logic.getBuildings(BuildingManager.SHOPTABLE), indexPos, CCollider.AUDIO_MATERIAL.WOOD)
    ...
    private async addChest(indexPos: cc.Vec3, mapDataStr: string) {
        let chest = this.addBuilding(await Logic.getBuildings(BuildingManager.CHEST), indexPos, CCollider.AUDIO_MATERIAL.WOOD)
    ...
    private async addVehicle(dungeon: Dungeon, mapDataStr: string, indexPos: cc.Vec3) {
        let n = this.addBuilding(await Logic.getBuildings(BuildingManager.VEHICLE), indexPos, CCollider.AUDIO_MATERIAL.METAL, '', true)
        let vehicle = n.getComponent(Vehicle)
        vehicle.init(dungeon, mapDataStr)
    }
    private async addDecorate(dungeon: Dungeon, mapDataStr: string, indexPos: cc.Vec3) {
        if (mapDataStr == '+++0') {
            this.addBuilding(await Logic.getBuildings(BuildingManager.GRASS01), indexPos, CCollider.AUDIO_MATERIAL.WOOD)
        } else if (mapDataStr == '+++1') {
            this.addBuilding(await Logic.getBuildings(BuildingManager.GRASS02), indexPos, CCollider.AUDIO_MATERIAL.WOOD)
        } else if (mapDataStr == '+++2') {
            this.addBuilding(await Logic.getBuildings(BuildingManager.GRASS03), indexPos, CCollider.AUDIO_MATERIAL.WOOD)
        } else if (mapDataStr == '+++3') {
            this.addBuilding(await Logic.getBuildings(BuildingManager.GRASS04), indexPos, CCollider.AUDIO_MATERIAL.WOOD)
        } else {
            let fd = this.addBuilding(await Logic.getBuildings(BuildingManager.DECORATIONFLOOR), indexPos, CCollider.AUDIO_MATERIAL.WOOD)
    ...
    private async addWaterBuilding(mapDataStr: string, indexPos: cc.Vec3) {
        if (this.hasThe(mapDataStr, '~#')) {
            let fint = parseInt(mapDataStr.substring(2))
            let co = this.addBuilding(await Logic.getBuildings(BuildingManager.COAST), indexPos, CCollider.AUDIO_MATERIAL.WOOD)
            ...
            let dn = this.addBuilding(await Logic.getBuildings(BuildingManager.WATERFALL), indexPos, CCollider.AUDIO_MATERIAL.WOOD)
            AudioPlayer.play(AudioPlayer.WATERFALL, false, true)
        }
    }
    private async addLamp(mapDataStr: string, indexPos: cc.Vec3) {
        ...
        let node = this.addBuilding(await Logic.getBuildings(prefabName), indexPos, CCollider.AUDIO_MATERIAL.WOOD)
        if (isOverHead) {
    ...
    private async addMartShelves(mapDataStr: string, indexPos: cc.Vec3) {
        //生成货架
        if (mapDataStr == MartShelves.TYPE_FRIDGE) {
            let ms = this.addBuilding(await Logic.getBuildings(BuildingManager.MARTFRIDGE), indexPos, CCollider.AUDIO_MATERIAL.WOOD).getComponent(MartShelves)
            ms.init(mapDataStr, this.getGoodsList(mapDataStr))
        } else {
            let ms = this.addBuilding(await Logic.getBuildings(BuildingManager.MARTSHELVES), indexPos, CCollider.AUDIO_MATERIAL.WOOD).getComponent(MartShelves)
            ms.init(mapDataStr, this.getGoodsList(mapDataStr))
        }
    }
    private async addExitDoor(mapDataStr: string, indexPos: cc.Vec3, exits: ExitData[]) {
        let prefab = await Logic.getBuildings(BuildingManager.EXITDOOR)
    ...
    /**添加空气墙 */
    public async addAirExit(mapData: string[][]) {
        let prefab = await Logic.getBuildings(BuildingManager.AIREXIT)
    ...
    private async addDoor(mapDataStr: string, indexPos: cc.Vec3, isDecorate: boolean) {
        let prefab = await Logic.getBuildings(BuildingManager.DOOR)
        let dir = parseInt(mapDataStr[3])
        let type = parseInt(mapDataStr.substring(1, 3))
    ...
    private async addDirWalls(mapDataStr: string, mapData: string[][], indexPos: cc.Vec3, levelData: LevelData, onlyShow: boolean) {
        let node: cc.Node = this.addBuilding(await Logic.getBuildings(BuildingManager.WALL), indexPos)
    ...
    private async addFurnitures(dungeon: Dungeon, mapDataStr: string, indexPos: cc.Vec3) {
        let data = new BuildingData()
        switch (mapDataStr) {
            case 'Z2':
        ...
        if (mapDataStr == 'Z3') {
            building = this.addBuilding(await Logic.getBuildings(BuildingManager.ROOMTV), indexPos)
            script = building.getComponent(Furniture)
            script.init(data, false)
        } else if (mapDataStr == 'Z4') {
            building = this.addBuilding(await Logic.getBuildings(BuildingManager.ROOMSOFA), indexPos, CCollider.AUDIO_MATERIAL.WOOD)
            building.zIndex = IndexZ.ACTOR
            script = building.getComponent(Furniture)
            script.init(data, false)
        } else if (mapDataStr == 'Z9') {
            building = this.addBuilding(await Logic.getBuildings(BuildingManager.ROOMSTOOL), indexPos)
            building.getComponent(RoomStool).init(dungeon)
            script = building.getComponent(Furniture)
            script.init(data, false)
        } else if (mapDataStr == 'Z10') {
            building = this.addBuilding(await Logic.getBuildings(BuildingManager.ROOMKITCHEN), indexPos)
            building.getComponent(RoomKitchen).init(indexPos)
            script = building.getComponent(Furniture)
            script.init(data, false)
        } else if (mapDataStr == 'Z17') {
            building = this.addBuilding(await Logic.getBuildings(BuildingManager.ROOMCLOCK), indexPos, CCollider.AUDIO_MATERIAL.WOOD)
            script = building.getComponent(Furniture)
            script.init(data, false)
        } else if (mapDataStr == 'Z18') {
            building = this.addBuilding(await Logic.getBuildings(BuildingManager.ROOMFISHTANK), indexPos, CCollider.AUDIO_MATERIAL.WOOD)
            building.getComponent(RoomFishtank).init(dungeon)
            script = building.getComponent(Furniture)
            script.init(data, false)
        } else if (mapDataStr == 'Z20') {
            building = this.addBuilding(await Logic.getBuildings(BuildingManager.ROOMWATERDISPENSER), indexPos, CCollider.AUDIO_MATERIAL.WOOD)
            script = building.getComponent(Furniture)
            script.init(data, false)
        } else if (mapDataStr == 'Z21') {
            building = this.addBuilding(await Logic.getBuildings(BuildingManager.ROOMTRASHCAN), indexPos, CCollider.AUDIO_MATERIAL.WOOD)
            script = building.getComponent(Furniture)
            script.init(data, false)
        } else {
            building = this.addBuilding(await Logic.getBuildings(BuildingManager.FURNITURE), indexPos, CCollider.AUDIO_MATERIAL.WOOD)
            script = building.getComponent(Furniture)
    ...
    /**掉落石头 */
    async addFallStone(pos: cc.Vec3, isAuto: boolean, withFire?: boolean) {
        if (!this.node) {
            return
        }
        let stone = this.addBuilding(await Logic.getBuildings(BuildingManager.FALLSTONE), pos)
    ...
    /**落雷 */
    async addLighteningFall(pos: cc.Vec3, isTrigger: boolean, needPrepare: boolean, showArea: boolean, damagePoint?: number) {
        if (!this.node) {
            return
        }
        let fall = this.addBuilding(await Logic.getBuildings(BuildingManager.LIGHTENINGFALL), Dungeon.getIndexInMap(pos))
    ...
    /**树根缠绕 */
    public async addTwineGrass(pos: cc.Vec3, isAuto: boolean) {
        if (!this.node) {
            return
        }
        let grass = this.addBuilding(await Logic.getBuildings(BuildingManager.DRYADTWINE), pos)
    ...
    /**幽光护盾 */
    public async addEnergyShield(player: Player, callback: (arg0: EnergyShield) => void): Promise<void> {
        if (!this.node) {
            if (callback) {
                callback(null)
            }
        }
        let shield = this.addBuilding(await Logic.getBuildings(BuildingManager.ENERGYSHIELD), player.pos)
