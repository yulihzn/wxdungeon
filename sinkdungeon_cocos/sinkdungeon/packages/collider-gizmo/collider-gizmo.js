...
class ColliderGizmo extends Editor.Gizmo {
        ...
        return {
            ...
            update: (dx, dy, event, type) => {
                ...
                if (type === ToolType.Center) {
                    ...
                    // 计算 radius
                    target.radius = position.sub(startOffset).mag()/this._view.scale;
                    // 防止 radius 小数点位数过多
    ...
    onCreateRoot () {
        ...
        this._tool.plot = (radius, position) => {
            ...
            dragArea.radius(radius * this._view.scale);
            circle.radius(radius * this._view.scale);
        };
    }
    ...
    onUpdate () {
        ...
        // 对齐坐标，防止 svg 因为精度问题产生抖动
        radius = Editor.GizmosUtils.snapPixel(radius) * this._view.scale;
